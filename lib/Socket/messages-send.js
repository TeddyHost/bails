...
const makeMessagesSocket = (config) => {
      ...
      await authState.keys.transaction(async () => {
         ...
         if (isGroup || isStatus) {
            ...
            if (senderKeyJids.length) {
               ...
               const result = await createParticipantNodes(senderKeyJids, senderKeyMsg, mediaType ? {
                  mediatype: mediaType
               } : undefined);
   ...
   const getButtonArgs = (message) => {
      ...
      } else if (message.interactiveMessage?.nativeFlowMessage && message.interactiveMessage.nativeFlowMessage?.buttons?.length > 0 && ['mpm', 'cta_catalog', 'send_location', 'call_permission_request', 'wa_payment_transaction_details', 'automated_greeting_message_view_catalog'].includes(message.interactiveMessage.nativeFlowMessage.buttons[0].name)) {
         return {
            tag: 'biz',
            attrs: {},
            content: [{
